...
class DataReader:
    ...
    def __init__(self, config, dataset_dir):
        ...
        self.config = config
        self.train, self.valid, self.test, self.user2feature, self.item2feature = self.load_data(dataset_dir)

    def load_data(self, dataset_dir):
        train_df = pd.read_csv(os.path.join(dataset_dir, "train_data.csv"))
        eval_df = pd.read_csv(os.path.join(dataset_dir, "eval_data.csv"))
        test_df = pd.read_csv(os.path.join(dataset_dir, "test_data.csv"))

        if self.config.use_explanations:
            train_explanations = pd.read_csv(os.path.join(dataset_dir, "train_explanations.csv"))
            eval_explanations = pd.read_csv(os.path.join(dataset_dir, "eval_explanations.csv"))
            test_explanations = pd.read_csv(os.path.join(dataset_dir, "test_explanations.csv"))
        ...
        data_df = pd.concat([train_df, eval_df, test_df], ignore_index=True)
        for i, row in data_df.iterrows():
            self.user_dict.add_entity(row['user_id'])
            self.item_dict.add_entity(row['item_id'])
            rating = row['rating']
        ...
        for i, row in train_df.iterrows():
            train.append({'user': self.user_dict.entity2idx[row['user_id']],
                         'item': self.item_dict.entity2idx[row['item_id']],
                         'rating': row['rating'],
                         'text': row['review'],
                         'feature': None})
        ...
        for i, row in eval_df.iterrows():
            valid.append({'user': self.user_dict.entity2idx[row['user_id']],
                         'item': self.item_dict.entity2idx[row['item_id']],
                         'rating': row['rating'],
                         'text': row['review'],
                         'feature': None})
        ...
        for i, row in test_df.iterrows():
            test.append({'user': self.user_dict.entity2idx[row['user_id']],
                         'item': self.item_dict.entity2idx[row['item_id']],
                         'rating': row['rating'],
                         'text': row['review'],
                         'feature': None})
