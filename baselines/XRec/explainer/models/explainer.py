...
class Explainer(torch.nn.Module):
    ...
    def forward(self, user_embedding, item_embedding, input_text):
        ...
        outputs = self.model(
            ...
            #attention_mask=tokenized_inputs["attention_mask"], # NEW
    ...
    def generate(self, user_embedding, item_embedding, input_text):
        ...
        cont = outputs[:, L:]
        texts = self.tokenizer.batch_decode(cont, skip_special_tokens=True, clean_up_tokenization_spaces=True)
        texts = [t.strip().lstrip('",. ').rstrip() for t in texts]
        return texts
