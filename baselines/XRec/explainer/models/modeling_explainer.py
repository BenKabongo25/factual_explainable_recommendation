...
# NEW
def add_embed_at_positions(tensor, pos, vec):
    if pos.dim() == 2:
        pos = pos[:, 0]
    B, S, D = tensor.shape
    mask = torch.nn.functional.one_hot(pos, num_classes=S).to(dtype=tensor.dtype, device=tensor.device)  # [B,S]
    mask = mask.unsqueeze(-1)  # [B,S,1]
    return tensor + mask * vec.unsqueeze(1)  # broadcast, no in-place
# END NEW
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # NEW
        if query_states.shape[1] > 1:
            query_states = add_embed_at_positions(query_states, user_embed_pos, user_embed)
            query_states = add_embed_at_positions(query_states, item_embed_pos, item_embed)
            key_states   = add_embed_at_positions(key_states,   user_embed_pos, user_embed)
            key_states   = add_embed_at_positions(key_states,   item_embed_pos, item_embed)
            value_states = add_embed_at_positions(value_states, user_embed_pos, user_embed)
            value_states = add_embed_at_positions(value_states, item_embed_pos, item_embed)
        # END NEW
