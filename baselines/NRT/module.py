...
class NRTEncoder(nn.Module):
    ...
    def forward(self, user, item):  # (batch_size,)
        ...
        if rating_one_hot.dim() == 1:
            rating_one_hot = rating_one_hot.unsqueeze(0)
...
class NRT(nn.Module):
    def __init__(self, nuser, nitem, ntoken, emsize, hidden_size, num_layers_mlp=4, max_r=5, min_r=1):
        super(NRT, self).__init__()
        self.encoder = NRTEncoder(nuser, nitem, emsize, hidden_size, num_layers_mlp, max_r, min_r)
        self.decoder = GRUDecoder(ntoken, emsize, hidden_size)
